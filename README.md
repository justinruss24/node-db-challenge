...
- [ ] Explain the difference between `Relational Databases` and `SQL`.
    SQL is a query language that helps users communicate with the database. A relational database represents a collection of related tables.
- [ ] Why do tables need a `primary key`?
    A primary key is used to uniquely identify rows for relational purposes.
- [ ] What is the name given to a table column that references the primary key on another table.
    a Foreign Key
- [ ] What do we need in order to have a _many to many_ relationship between two tables.
    A third intermediary table with foreign keys connecting the two main tables.
